...
from utils.util import params_add_defaults
...
def plot_patches_conv(run_path, train_args, solver_args):
    ...
    coeff_collapse = np.zeros(len(load_list))
    #code_list = np.zeros((len(load_list), val_patches.shape[0], final_phi.shape[1]))
    code_list = np.zeros((len(load_list), val_patches.shape[0], dict_size * patch_size1 * patch_size2))
    recovered_dict = np.zeros((len(load_list), *final_phi.shape))
    ...
    for idx, method in enumerate(load_list):
        ...
        if solver_args.solver != 'FISTA':
            ...
            # encoder.load_state_dict(torch.load(run_path  + f"/encoderstate_epoch{method}.pt")['model_state'])
            encoder = torch.load(run_path  + f"/encoder_epoch{method}.pt")
            encoder = encoder.to(default_device)
            encoder.ramp_hyperparams()

        # np.load(train_args.save_path + 'train_savefile.npz')['phi'][method - 1]
        dictionary = torch.load(run_path  + f'/dictionary_epoch{method}.pt')

        # kl_collapse_count = np.zeros(final_phi.shape[1])
        # coeff_collapse_count = np.zeros(final_phi.shape[1])
        kl_collapse_count    = np.zeros((dict_size, patch_size1, patch_size2))
        coeff_collapse_count = np.zeros((dict_size, patch_size1, patch_size2))
        ...
        for i in range(val_patches.shape[0] // train_args.batch_size):
            patches = val_patches[i * train_args.batch_size:(i + 1) * train_args.batch_size]   # removed .T
            ...
            # Make data 4D, adding channels axis
            ...
            #     kl_collapse_count[k] += (kl_loss[:, k] <= 1e-2).sum() / val_patches.shape[0]
            #     coeff_collapse_count[k] += (np.abs(code_est[:, k]) <= 1e-2).sum() / val_patches.shape[0]
            # Mean across all validation
            kl_collapse_count    += (kl_loss <= 1e-2).type(torch.float).sum(dim=0, dtype=torch.float).detach().cpu().numpy() / val_patches.shape[0]
            coeff_collapse_count += (np.abs(code_est) <= 1e-2).sum(axis=0) / val_patches.shape[0]
            
            #code_list[idx, i*train_args.batch_size:(i+1)*train_args.batch_size] = code_est
            # reshape (100, 6, 16, 16) => (100, 1536)
            code_list[idx, i*train_args.batch_size:(i+1)*train_args.batch_size] = code_est.reshape((code_est.shape[0], -1))

        # TODO: Can't do this for convolutional? Dimensions do not match, patch=16x16, code=6x16x16
        # C_sr = (val_patches.T @ code_list[idx]) / len(val_patches)
        # C_rr = (code_list[idx].T @ code_list[idx]) / len(val_patches)

        # posterior_collapse[idx] = 100. * (kl_collapse_count >= 0.95).sum() / phi.shape[1]
        # coeff_collapse[idx] = 100. * (coeff_collapse_count >= 0.95).sum() / phi.shape[1]
        posterior_collapse[idx] = 100 * (kl_collapse_count >= 0.95).mean()
        coeff_collapse[idx] = 100 * (coeff_collapse_count >= 0.95).mean()
